 ...
 * has no ASCII (7-bit) nor ISO-Latin-1 (8-bit) representation.
 *
 ...
 * (and thus get your platform's default charset used for the
 * bytes-to-chars conversion) and you get garbage.  Then it is likely
 * that the file was produced using some other character encoding, and
 * the code below shows how you successful read it in.
 */
...
public class CharacterEncodings {
	...
	static public void main( String[] args ) {
		...
		/*
		  Some of the more popular character sets, listed by their name.
		  These are required to be supported by all JVMs
		*/
		...
		/*
		  First write out pi using a simple FileWriter, paying no
		  attention to charsets.  No need for PrintWriter since we
		  already have a string.  The charset used will be the JVM's
		  default, likely UTF8.
		*/
		...
		/*
		  Now write out pi with various standard character
		  encodings. To get the exact charset we want, we must use an
		...
		*/
		...
		for( String name : popular ) {
			try {
				...
				System.out.println( "Roundtrip " + f + " worked: " + success );
			} catch( UnsupportedEncodingException never ) {
			} catch( IOException ioe ) {
				System.err.println( ioe );
			}
